...
% noise_o1 = zeros(1,151);
% noise_o2 = zeros(1,151);
...
if (ramp_only)
    ...
    u_lb=repmat([20*ones(dim_speed,1);0.2*ones(dim_ramp,1)],1,Nc);
...
%%
for i=1:N/M
    objfun=@(co) ModelState_Noise(x, [v_control; co], Nc, Np, para, [v_control_pre;u_pre(:,1)],scenario);
    confun=@(co) ModelCons_Noise(x, [v_control; co], Nc, Np, M, scenario);
