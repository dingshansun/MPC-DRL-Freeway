function [InitialObservation,LoggedSignals] = RL_MPC_resetfnc
...
u_mpc=MPC_imp_(x,repmat([70 70 1]',1,2),scenario);
u_imp=u_mpc;
% u_mpc=1;
...
% LoggedSignals.u_pre=1; %% initial previous actions
LoggedSignals.x=x(1:22);
LoggedSignals.k=k;
LoggedSignals.TTS=TTS;
LoggedSignals.noiseo1=noise_o1;
LoggedSignals.noiseo2=noise_o2;
InitialObservation=[x(1:22)./norm_x; (demando1(k,scenario)+noise_o1(ceil((k-59)/6)))/1000; (demando2(k,scenario)+noise_o2(ceil((k-59)/6)))/1000; u_mpc(:,1)./[102 102 1]'; u_imp(:,1)./[102 102 1]'];
end